 ...
 *      This was always defaulting to Compressor32() even on a 64bit machine. Why?!!
 *      turns out, both this STATIC (static Compression()), and the STATIC Is64Bit must be initialized in a particular order,
 *      but there is no such guarantee. For us, it was working against us. this static was being initialized
 *      before the Is64Bit static, and so was always defaulting to Compressor32() and subsiquently blowing up.
 *      We explicitly check the pointer size to make the right things happen. But it's scary to think how many
 *      other places this kind of logic may exist.
 */
#if true
...
namespace Server.Network
{
    ...
    public sealed class Compressor64 : ICompressor
    {
        internal class SafeNativeMethods
        {
             ...
             */
#if true
            // 12/3/21: RunUO's crap never worked (2)
            //  you need to return an IntPtr
        ...
        public string Version
        {
            ...
            {
                //return SafeNativeMethods.zlibVersion();
                // 12/3/21: RunUO's crap never worked (3)
                //  you need to Marshal the return data
...
namespace Server.Network
{
    ...
    public static class Compression
    {
        ...
        static Compression()
        {
            ...
            // 12/3/21: RunUO's crap never worked (1)
            //  This was always defaulting to Compressor32() even on a 64bit machine. Why?!!
            //  turns out, both this STATIC class, and the STATIC Is64Bit must be initialized in a particular order,
            //  but there is no such guarantee. For us, it was working against us. this static was being initialized
            //  before the Is64Bit static, and so was always defaulting to Compressor32() and subsiquently blowing up.
            //  We explicitly check the pointer size to make the right things happen. But it's scary to think how many
            //  other places this kind of logic may exist.
            else if (Core.Is64Bit || IntPtr.Size == 8)
            {
                Compressor = new Compressor64();
            }
            else
