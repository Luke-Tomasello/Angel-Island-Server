...
namespace Server.Network
{
    ...
    // Angel Island 5 introduced the notion of a 'MakersMark'
    //  We will override RunUO2.6 here to support this functionality.
#if true
    public class EquipmentInfo
    {
        ...
        public EquipmentInfo(int number, string crafter, bool unidentified, EquipInfoAttribute[] attributes)
        {
            m_Number = number;
            m_Crafter = crafter;
    ...
    public sealed class DisplayEquipmentInfo : Packet
    {
            ...
            : base(0xBF)
        {
            ...
            this.EnsureCapacity(17 + (info.Crafter == null ? 0 : 6 + info.Crafter == null ? 0 : info.Crafter.Length) + (info.Unidentified ? 4 : 0) + (attrs.Length * 6));
    ...
    public sealed class DisplayEquipmentInfo : Packet
    {
        public DisplayEquipmentInfo(Item item, EquipmentInfo info) : base(0xBF)
        {
            ...
            this.EnsureCapacity(17 + (info.Crafter == null ? 0 : 
                6 + info.Crafter.Name == null ? 0 :
                info.Crafter.Name.Length) + 
                (info.Unidentified ? 4 : 0) + 
                (attrs.Length * 6));
    ...
    public sealed class MobileStatusExtended : Packet
    {
        ...
        public MobileStatusExtended(Mobile m, NetState ns) : base(0x11)
        {
            ...
            if (type >= 5)
            {
                ...
                m_Stream.Write((byte)(1/*m.Race.RaceID + 1*/));  // Would be 0x00 if it's a non-ML enabled account but...
            }

            m_Stream.Write((short)m.StatCap);

            m_Stream.Write((byte)m.FollowerCount);
        ...
        public MobileStatus(Mobile beholder, Mobile beheld, NetState ns) : base(0x11)
        {
            ...
            if (type > 0)
            {
                ...
                if (type >= 5)
                {
                    ...
                    m_Stream.Write((byte)(1/*beheld.Race.RaceID + 1*/)); // Would be 0x00 if it's a non-ML enabled account but...
                }

                m_Stream.Write((short)beheld.StatCap);

                m_Stream.Write((byte)beheld.FollowerCount);
...
namespace Server.Network
{
    ...
    public sealed class CharacterList : Packet
    {
            ...
            : base(0xA9)
        {
            ...
            {   // not sure what's going on here, but cityInfo was null.
                // I suspect there was an error in login, maybe another character was already logged in from that account
                //  the the error didn't abort the character creation correctly and continued on and we got here.
                //  I'm trying to reproduce in hopes I can resolve the issue that lead us here.
                Console.WriteLine("Failure in {CharacterList : Packet}");
            }
        }
    ...
    public enum ALRReason : byte
    {   // don't change/add to these since thy are sent to the client
        //  (AccountLoginRej : Packet) below
        Invalid = 0x00,
        InUse = 0x01,
        Blocked = 0x02,
        BadPass = 0x03,
    ...
    public sealed class ServerInfo
    {
        ...
        public ServerInfo(string name, int fullPercent, TimeZone tz, IPEndPoint address)
        {
            m_Name = name;
            m_FullPercent = fullPercent;
            m_TimeZone = tz.GetUtcOffset(DateTime.UtcNow).Hours;
            m_Address = address;
        }
    }
