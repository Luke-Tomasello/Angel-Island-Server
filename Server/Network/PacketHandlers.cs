 ...
 *	ChangeLog:
 *  11/24/22. Adam: crash log #1
 *  Still researching this.
 *      I believe one of IAccount a or CityInfo[] info was somehow null. Maybe he closed the client at the city list
 *      or shortly before? 
 *      I added additional null checks before the call to send the packet. If any of the key data is null, I Dispose of the 
 *      connection, and log the error.
 * Clients:
 * - Count: 1
 * + 24.60.22.100: (account = chappelle4life)
 * Exception:
 * System.NullReferenceException: Object reference not set to an instance of an object.
 *    at Server.Network.CharacterListOld..ctor(IAccount a, CityInfo[] info) in C:\Users\luket\Documents\Software\Development\Product\Src\Angel Island\Shard.AI\Server\Network\Packets.cs:line 4317
 *    at Server.Network.PacketHandlers.GameLogin(NetState state, PacketReader pvSrc) in C:\Users\luket\Documents\Software\Development\Product\Src\Angel Island\Shard.AI\Server\Network\PacketHandlers.cs:line 2677
 *    at Server.Network.MessagePump.HandleReceive(NetState ns) in C:\Users\luket\Documents\Software\Development\Product\Src\Angel Island\Shard.AI\Server\Network\MessagePump.cs:line 150
 *    at Server.Network.MessagePump.Slice() in C:\Users\luket\Documents\Software\Development\Product\Src\Angel Island\Shard.AI\Server\Network\MessagePump.cs:line 135
 *    at Server.Core.Main(String[] args) in C:\Users\luket\Documents\Software\Development\Product\Src\Angel Island\Shard.AI\Server\Main.cs:line 1769
 *
 * 11/17/22, Adam
 *	    I added AI style IsValidAuthID() functionality which skips the verification if we are not
 *	    on the Login Server. I.e., you had to have passed authentication there before getting here (game server).
 *	    (This is a redo of the code I put in 11/13/10 since we've recently upgraded our networking to RunUO 2.6)
 *	10/31/22, Adam (TargetResponse)
 *  The #if'ed out code in (TargetResponse) exists in both RunUO2.6 and RunUO2.7
 *  In both of these RunUO versions, targeting the floor of a house fails, 
 *  i.e., [get z
 *  This works in Angel Island 6, but begun failing when I moved us over to
 *  RunUO2.6. Apparently, and I've seen this elsewhere, RunUO2.6 did away with masking
 *  the tileid. (I think it was in basemulti) Presumably, they handle it 
 *  differently now, but in any case, targeting the floor of a multi fails in 
 *  even their latest version. 
 *  Revert: I'm reverting this line to Angel Island 6 style of masking which 
 *  solves the problem. I think this was just a case they missed. We'll need to keep a watchful eye
 *  for other masking issues.
 *  8 /1/22, Adam
 *	    Update Console messages to Yellow for "Client:" messages.
 *  1/11/22, Adam: Added "&& !SpecialStaffException()"
 *      to allow staff to have more than one player character in-world at the same time.
 *  2/4/11, Adam
...
namespace Server.Network
{
    ...
    public static class PacketHandlers
    {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    for (int i = 0; !valid && i < tiles.Length; ++i)
                                    {
                                        ...
                                        /* 10/31/22, Adam
                                         * The #if'ed out code below exists in both RunUO2.6 and RunUO2.7
                                         * In both of these RunUO versions, targeting the floor of a house fails, 
                                         *  i.e., [get z
                                         *  This works in Angel Island 6, but begun failing when I moved us over to
                                         *  RunUO2.6.
                                         *  Apparently, and I've seen this elsewhere, RunUO2.6 did away with masking
                                         *  the tileid. (I think it was in basemulti) Presumably, they handle it 
                                         *  differently now, but in any case, targeting the floor of a multi fails in 
                                         *  even their latest version. 
                                         *  Revert: I'm reverting this line to Angel Island 6 style of masking which 
                                         *  solves the problem.
                                         *  I think this was just a case they missed. We'll need to keep a watchful eye
                                         *  for other masking issues.
                                         */
#if RunUO2_6
        ...
        #region RunUO2.6 Port compatibility functions
        private static bool IsValidAuthID(NetState state, int authID)
        {
        ...
        public static void GameLogin(NetState state, PacketReader pvSrc)
        {
            ...
            if (e.Accepted)
            {
                ...
                {   // see crash log #1 at the end of this file
                    if (state != null && state.Account != null && state.CityInfo != null)
                        state.Send(new CharacterListOld(state.Account, state.CityInfo));
                    else
                    {
                        LogHelper logger = new LogHelper("network failure.log", false, false, false);
                        logger.Log(LogType.Text, "state.Send(new CharacterListOld(state.Account, state.CityInfo)) : PacketHandlers.cs");
                        logger.Log(LogType.Text, string.Format("state.Account = {0}, state.CityInfo = {1}",
                            (state != null && state.Account != null) ? state.Account : "-null-",
...
namespace Server.Network
{
    ...
    public class PacketHandlers
    {
        ...
        public static void UnicodeSpeech(NetState state, PacketReader pvSrc)
        {
            ...
            //Pix: give older clients the ability to use the guild chat system.
            ...
            //Pix: give older clients the ability to use the allied chat system
            if (text.Length > 0 && text[0] == '|')
            {
                text = text.Substring(1);
        ...
        public static void ContextMenuRequest(NetState state, PacketReader pvSrc)
        {
            ...
            if (!Core.RuleSets.AngelIslandRules() && !Core.RuleSets.MortalisRules() && PublishInfo.Publish < 14)
                return;
        ...
        private static bool SpecialStaffException(IAccount a)
        {
            return !(a is Account acct && acct != null && acct.AccessLevel <= AccessLevel.GameMaster);
        }
        public static void PlayCharacter(NetState state, PacketReader pvSrc)
        {
            pvSrc.ReadInt32(); // 0xEDEDEDED
            ...
            else
            {
                ...
                for (int i = 0; i < a.Length; ++i)
                {
                    ...
                    // 1/11/22, Adam: Added "&& !SpecialStaffException()
                    //  to allow staff to have more than one player character in-world at the same time.
                    if (check != null && check.Map != Map.Internal && check != m && !SpecialStaffException(a))
                    {
                        Console.WriteLine("Login: {0}: Account in use", state);
                        state.Send(new PopupMessage(PMMessage.CharInWorld));
                        return;
