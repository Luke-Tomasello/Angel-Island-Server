    ...
    public class Timer
    {
        ...
        public DateTime Next
        {
            get
            {
                try
                {
                    return DateTime.UtcNow + TimeSpan.FromMilliseconds(m_Next - Core.TickCount);
                }
                catch
                {   // handle overflow.
                    //  Here I try to set it to the largest representable value
                    double MaxDT = DateTime.MaxValue.Ticks;
                    double checkTS = TimeSpan.MaxValue.Ticks;
                    double checkNow = DateTime.UtcNow.Ticks;
                    double offset = 0;
                    if (checkNow + (m_Next - Core.TickCount) > MaxDT)
                        offset = checkTS - checkNow;
                    //else
                    //offset = (MaxDT - checkNow) / 2;

                    return DateTime.UtcNow + TimeSpan.FromMilliseconds(offset);
                }
            }
            // Old RunUO code. blows up with an overflow now and then. 
            // Obnoxious
        ...
        public static Queue<Timer> Queue { get { return m_Queue; } }
        ...
        public DateTime NextTick { get { return Next; } }
