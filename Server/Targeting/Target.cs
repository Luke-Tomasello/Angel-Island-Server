...
/* Server\Targeting\Target.cs
 * ChangeLog
 *	8/22/07
 *		Redesign the change below to make use of the creatures CanTarget property.
 *			We now return false for CanTarget if the creature is blessed.
 *	9/22/05, Adam
 *		Players can no longer target Blessed, non-human, creatures that are in the zoo. 
 *		This is so we can have the Britannia Royal Zoo filled with creatures 
 *		players can not train off of.
 */
    ...
    public abstract class Target
    {
        ...
        public void Invoke(Mobile from, object targeted)
        {
            ...
            else if (targeted is Mobile)
            {
                ...
                else if (((Mobile)targeted).CanTarget == false && from.AccessLevel == AccessLevel.Player)
                {
