 ...
 *  10/7/22, Adam
 *      Port to RunUO 2.6
 *      In MultiComponentList(BinaryReader reader, int count)
 *      Add a AI 5/6 compatibility conversion to make RunUO 2.6 MCL lists Ai compatible. 
 *      (See comments)
 *	5/22/08, Adam
 *		mcl.add() added the ignoreRoof override for our StaticHouses
 *	6/25/06, Adam
 *		format cleanup
 *  06/08/07, plasma
...
namespace Server
{
    ...
    public sealed class MultiComponentList
    {
        ...
        public Point2D Min { get { return m_Min; } set { m_Min = value; } }
        public Point2D Max { get { return m_Max; } set { m_Max = value; } }
        ...
        public StaticTile[][][] Tiles { get { return m_Tiles; } set { m_Tiles = value; } }
        public MultiTileEntry[] List { get { return m_List; } set { m_List = value; } }
        public void Add(int itemID, int x, int y, int z)
        {
            Add(itemID, x, y, z, false);
        }
...
#if TODO
                    // RunUO 2.6
                    // RunUO 2.6's code generated very large MCL rects, likely because the bit of code that reinterprets it
                    //  hasn't been porter over yet. I'm nervous to replace all our MCL code wholesale as we have custom housing
                    //  that will likely complicate the port.
                    tiles[xOffset][yOffset].Add((ushort)allTiles[i].m_ItemID, (sbyte)allTiles[i].m_OffsetZ);
#else
                    // AI 5.0
                    //we add this AI style conversion from RunUO 2.6 => AI 5/6 so that our MCLs are compatible with the rest of the system.
                    //  at some point, we should revisit what RunUO was accomplishing here, and try to port to their model.
                    // AI 5.0 change ------------VVVVVV short to ushort
                    tiles[xOffset][yOffset].Add((ushort)((allTiles[i].m_ItemID & 0x3FFF) | 0x4000), (sbyte)allTiles[i].m_OffsetZ);
#endif
                }
            }
