...
/* Server\Skills.cs
 * CHANGELOG
 * 9/28/21, Adam: (OnAfterUseSkill)
 * Add a call to from.OnAfterUseSkill() to inform the mobile what skill they used.
 * PlayerMobile now enumerates their pets and informs them of the skill used. By default, there is no action.
 * Usage: familiars intuit their masters wishes. Example: when the Basilisk (a familiar)
 * is following her master, and her master hides, the basilisk will also hide.
 */

/* Re: Real Skill vs. Shown Skill
 * Post by Rammar � Thu Jun 24, 2010 1:11 am
 ...
 * https://forums.uosecondage.com/viewtopic.php?t=19805
 */
...
namespace Server
{
            ...
            set
            {
                ...
                if (m_Base != sv)
                {
                    ...
                    if (m != null)
                    {
                        ...
                        m.OnSkillChange(SkillName, (double)oldBase / 10, (double)m_Base / 10);
                    }
                }
            }
        }
    ...
    public class SkillInfo
    {
        ...
        private static SkillInfo[] m_Table = new SkillInfo[49] //[55]
			{//                 skillID,    name,                       strScale,   dexScale,   intScale,   title,          Callback,   strGain,    dexGain,    intGain,    gainFactor
            ...
            if (skillID >= 0 && skillID < SkillInfo.Table.Length)
            {
                ...
                if (info.Callback != null)
                {
                    if (Core.TickCount - from.NextSkillTime >= 0 && from.Spell == null)
                    {
                        ...
                        from.OnAfterUseSkill((SkillName)skillID);
