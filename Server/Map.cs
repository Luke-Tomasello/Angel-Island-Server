 ...
 *  3/19/07, Adam
 *      Recfactor CanFit to use Utility.CanFitFlags.
...
namespace Server
{
    ...
    public sealed class Map : IComparable, IComparable<Map>
    {
        ...
        public static Map Parse(string value)
        {
            ...
            // 3/7/2024, Adam: need a programmatic way to set map to null via reflection
            if (value == null)
                return null;
        ...
        public bool CanSpawnLandMobile(Point3D p)
        {
            return CanSpawnLandMobile(p.m_X, p.m_Y, p.m_Z);
        }

        public bool CanSpawnLandMobile(Point2D p, int z)
        {
            return CanSpawnLandMobile(p.m_X, p.m_Y, z);
        }

        public bool CanSpawnLandMobile(int x, int y, int z)
        {
        ...
        public object GetTopSurface(Point3D p)
        {
            ...
            return GetTopSurface(p, out surfaceZ);
        }
        ...
        public object GetTopSurface(Point3D p, out int surfaceZ)
        {
            surfaceZ = int.MinValue;
        ...
        // 10/10/22, Adam: Don't try to sort this dictionary. Microsoft states "The order in which the items are returned is undefined."
        //  https://bit.ly/3fVmyTk
        //  If you needed sorted (on priority) access to the Maps regions, use RegionsSorted() below
        //  Also note, all the find functions look through the sector's regions, and those are kept sorted. 
        ...
        // 10/10/22, Adam: see note above.
        List<Region> m_sortedRegions = new();
        public List<Region> RegionsSorted
        {
            get
            {
                if (m_sortedRegions.Count == 0)
                    m_sortedRegions = m_Regions.Values.OrderByDescending(o => o.Priority).ToList<Region>();
                return m_sortedRegions;
            }
        }
        ...
        private Point3D GetGo(Region reg, Map map)
        {
            Point3D p3d = reg.GoLocation;
            if (p3d == Point3D.Zero && reg.Coords.Count > 0)
                try
                {
                    p3d = new Point3D(reg.Coords[0].Start.X + 1, reg.Coords[0].Start.Y + 1, GetAverageZ(reg.Coords[0].Start.X, reg.Coords[0].Start.Y));
                }
                catch
                {
                    ;
