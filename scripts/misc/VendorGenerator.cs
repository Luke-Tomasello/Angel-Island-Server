...
/* Scripts/Misc/VendorGenerator.cs
 * CHANGELOG:
 *	04/11/07 - Pix
...
namespace Server
{
    public class VendorGenerator
    {
        ...
        private static bool GetFloorZ(Map map, int x, int y, out int z)
        {
            ...
            if (IsFloor(lt.ID) && Utility.CanFit(map, x, y, lt.Z, 16, Utility.CanFitFlags.requireSurface))
            {
                z = lt.Z;
                return true;
            }
            ...
            for (int i = 0; i < tiles.Length; ++i)
            {
                ...
                if (IsStaticFloor(t.ID) && Utility.CanFit(map, x, y, t.Z + (id.Surface ? id.CalcHeight : 0), 16, Utility.CanFitFlags.requireSurface))
                {
