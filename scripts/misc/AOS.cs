 ...
 *	5/8/07, Pix
 *		Added SE/ML support (for custom houses)
 *  11/17,06, Adam
 *      Add: #pragma warning disable 429
 *      The three Unreachable code complaints in this file are acceptable
 *      C:\Program Files\RunUO\Scripts\Misc\AOS.cs(51,41): warning CS0429: Unreachable expression code detected
 *	7/26/05, Adam
 *		Massive AOS cleanout
 *  6/5/04, Pix
 *		Merged in 1.0RC0 code.
 *	5/10/2004, pixie
 *		Turned LBR animations back on.
 *  4/30/2004, pixie
 *    changed and commented SupportedFeatures.Value
 *    enabled AOS animations and disabled LBR animations
 */
...
namespace Server
{
    public class AOS
    {
        ...
        {
            return Damage(m, from, damage, phys, fire, cold, pois, nrgy, false, source_weapon);
        }
        ...
        public static int Damage(Mobile m, Mobile from, int damage, int phys, int fire, int cold, int pois, int nrgy, bool keepAlive, object source_weapon)
        {
            ...
            if (phys == 0 && fire == 100 && cold == 0 && pois == 0 && nrgy == 0)
            {
                ...
                Engines.Apiculture.BeeSwarm.EndEffect(m, true);
            }

            // Adam: only valid return path
            //if ( !Core.AOS )
            {
                m.Damage(damage, from, source_weapon);
                return damage;
            }
            /*
						Fix( ref phys );
...
#region Dragon Barding
        ...
        {
            return Damage(m, null, damage, phys, fire, cold, pois, nrgy, source_weapon);
        }
    ...
    public abstract class BaseAttributes
    {
        ...
        {
            if ((value == (int)AosWeaponAttribute.DurabilityBonus) && (this is AosWeaponAttributes))
            {
                if (m_Owner is BaseWeapon)
                    ((BaseWeapon)m_Owner).UnscaleDurability();
            }
            else if ((value == (int)AosArmorAttribute.DurabilityBonus) && (this is AosArmorAttributes))
            {
