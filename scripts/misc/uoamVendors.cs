...
namespace Server
{
    public class UOAMVendorGenerator
    {
        ...
        {
            CommandSystem.Register("UOAMVendors", AccessLevel.Owner, new CommandEventHandler(Generate_OnCommand));
        }
        ...
        {
            string vendor_path = Path.Combine(Core.DataDirectory, "Common.map");
            m_Count = 0;
        ...
        public static int GetSpawnerZ(int x, int y, Map map)
        {
            ...
            for (int i = 1; i <= 20; ++i)
            {
                if (Utility.CanFit(map, x, y, z + i, 16, Utility.CanFitFlags.requireSurface))
                    return z + i;
