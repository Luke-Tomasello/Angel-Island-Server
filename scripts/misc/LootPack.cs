 ...
 *      Add new Dynamic Loot Packs.
 *      These dynamic loot packs are Dynamic Attributes usually added by our Spawners.
 *      You use this system to dynamically associate extended loot with any creature.
 */
...
namespace Server
{
    ...
    public class LootPackEntry
    {
        ...
        {
            // 11/1/21: adam, turned off for safety
            if (item != null && false)
            {
                /*if (item is BaseWeapon && 1 > Utility.Random(100) && !Core.AngelIsland)
				{
					item.Delete();
					item = new FireHorn();
					return item;
                ...
                if (item is BaseWeapon || item is BaseArmor || item is BaseJewel)
                {
                    ...
                    else // not aos
                    {
                        ...
                        else if (item is BaseArmor)
                        {
                            ...
                            if (40 > Utility.Random(100))
                                armor.DurabilityLevel = (ArmorDurabilityLevel)GetRandomOldBonus();
                        }
                    }
                }
