 ...
 *  9/29/22, Adam
 *      Merge in RunUO 2.6
 *      Add filtering on map and rectangle
 *	04/11/07 - Pix
 *		Removed decorating other facets.
 */
...
namespace Server
{
    public class DoorGenerator
    {
        ...
        {
            CommandSystem.Register("DoorGen", AccessLevel.Owner, new CommandEventHandler(DoorGen_OnCommand));
        }
        ...
        public static void Generate(List<Rectangle2D> rects = null, Map[] maps = null)
        {
            ...
            for (int j = 0; j < maps.Length; j++)
            {
                ...
                if (m_Map == Map.Trammel)
                {
                    ...
                    for (int i = 0; i < m_BritRegions.Length; ++i)
                        Generate(m_BritRegions[i], rects);

                    trammelCount = m_Count;
                }
                else if (m_Map == Map.Felucca)
                {
                    m_Count = 0;
                ...
                else if (m_Map == Map.Ilshenar)
                {
                    ...
                    for (int i = 0; i < m_IlshRegions.Length; ++i)
                        Generate(m_IlshRegions[i], rects);

                    ilshenarCount = m_Count;
                }
                else if (m_Map == Map.Malas)
                {
                    ...
                    for (int i = 0; i < m_MalasRegions.Length; ++i)
                        Generate(m_MalasRegions[i], rects);
        ...
        public static BaseDoor AddDoor(int x, int y, int z, DoorFacing facing)
        {
            ...
            if (!Utility.CanFit(m_Map, x, y, z, 16, Utility.CanFitFlags.requireSurface))
            {
                //LogHelper logger2 = new LogHelper("doorstry.log", false, true, true);
                //logger2.Log(LogType.Text, new Point3D(x, y, z));
                //logger2.Finish();
                return null;
            }
            ...
            LogHelper logger = new LogHelper("doors.log", false, true, true);
            logger.Log(LogType.Item, door);
            logger.Finish();
        ...
        public static void Generate(Rectangle2D region, List<Rectangle2D> rects = null)
        {
            for (int rx = 0; rx < region.Width; ++rx)
            {
                for (int ry = 0; ry < region.Height; ++ry)
                {
                    ...
                    for (int i = 0; i < tiles.Length; ++i)
                    {
                        ...
                        if (rects != null && !R2dListContains(rects, new Point3D(vx, vy, z)))
                            continue;
