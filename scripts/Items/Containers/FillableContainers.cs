 ...
 * CHANGELOG:
 *  7/16/2023, Adam
 *      rework system to be more consistent with spawners
 *      1. Add ContentType.Rare
 *      2. Add settable MinDelay/MaxDelay a la Spawners
 *      3. Add MustSteal property for all items in container.
 *      4. Add TotalRespawn
 *	2/28/11, adam
...
namespace Server.Items
{
    public abstract class FillableContainer : LockableContainer
    {
        ...
        public override bool CanFreezeDry { get { return false; } } // will continue to refill if allowed to freezedry
        #endregion
#endif
        ...
        //        public virtual int MinDelay.FromMinutes { get { return ContentType != FillableContentType.Rare ? 60 : (int)TimeSpan.FromHours(12).TotalMinutes; } }
        //        public virtual int MaxDelay.FromMinutes { get { return ContentType != FillableContentType.Rare ? 90 : (int)TimeSpan.FromHours(24).TotalMinutes; } }
        ...
        protected FillableContent m_Content = null;
        protected DateTime m_NextRespawnTime = DateTime.MinValue;
        protected Timer m_RespawnTimer = null;
        private TimeSpan m_MinDelay = TimeSpan.FromMinutes(60);
        private TimeSpan m_MaxDelay = TimeSpan.FromMinutes(90);
        private bool m_mustSteal = false;

        [CommandProperty(AccessLevel.GameMaster, AccessLevel.Seer)]
        public virtual bool MustSteal { get { return m_mustSteal; } set { m_mustSteal = value; InvalidateProperties(); } }
        ...
        public bool TotalRespawn
        {
            ...
            set
            {
                if (value == true)
                {
                    ...
                    m_NextRespawnTime = DateTime.MinValue;  // it's time
                    Respawn();                              // generate objects
                    CheckRespawn();                         // resets the respawn time
                }
            }
        }
        ...
        {
            int count = 0;
            if (this.ContentType == FillableContentType.Rare)
            {
        ...
        public void CheckRespawn()
        {
            ...
            if (canSpawn)
            {
                if (m_RespawnTimer == null)
                {
                    int mins = Utility.RandomMinMax((int)this.MinDelay.TotalMinutes, (int)this.MaxDelay.TotalMinutes);
        ...
        public virtual void GenerateContent()
        {
            ...
            for (int i = 0; i < toSpawn; ++i)
            {
                ...
                if (this.ContentType != FillableContentType.Rare)
                {
                    item = m_Content.Construct();
                    if (MustSteal)
                        item.SetItemBool(ItemBoolTable.MustSteal, true);
                }
                else
                {
                    item = Loot.RareFactoryItem(1.0);
                    if (MustSteal)
                        item.SetItemBool(ItemBoolTable.MustSteal, true);
                }
    ...
    {
        public static List<Type> Registry = new();
        ...
        public virtual Item Construct()
        {
            ...
            else if (item is SmallFish)
                item.ItemID = Utility.RandomList(0x0DD6, 0x0DD7);
    ...
    public class FillableContent
    {
        ...
        public static FillableContent Rare = new FillableContent(
            1,
            new Type[]
            {
        ...
        public static FillableContent ArtisanGuild = new FillableContent(
            ...
            new FillableEntry[]
            {
                ...
                new FillableEntry( 2, typeof( Statue1 ) ),
                new FillableEntry( 1, typeof( Statue2 ) ),
                new FillableEntry( 2, typeof( Statue3 ) ),
                new FillableEntry( 2, typeof( Statue4 ) ),
                new FillableEntry( 2, typeof( Bust ) ),
        ...
        public static FillableContent Docks = new FillableContent(
            ...
            new FillableEntry[]
            {
                ...
                new FillableEntry( 2, typeof( SmallFish ) ),
        ...
        public class BookEntry : FillableEntry
        {
            public BookEntry(int weight)
                : base(weight, typeof(RedBook))
            {
            }

            public override Item Construct()
            {
                if (LibraryBookCache.LibraryDropRate > 0.0 && Utility.RandomDouble() < LibraryBookCache.LibraryDropRate)
                {
                    BaseBook book = LibraryBookCache.Generate();

                    if (book != null)
                        return book;
                }

                return Loot.Construct(Loot.LibraryBookTypes);
            }
        }
        ...
        public static FillableContent Stables = new FillableContent(
            ...
            new FillableEntry[]
            {
                new FillableEntry( 1, typeof( WheatSheaf ) ), // OSI uses 0x0C55 as graphic
                new FillableEntry( 1, typeof( Carrot ) ),
                new FillableEntry( 1, typeof( Hay ) ), // Not OSI accurate
            });
