 ...
 * ChangeLog:
 *  1/16/22, Adam (OnPlacement)
 *      players trapping players (during PvP) in house previews. Stop it!
 *          We no longer allow dropping a preview on a mobile.
 *	2/28/10. Adam
...
namespace Server.Items
{
    ...
    public class SurveyToolEntry
    {
        ...
        public bool OnPlacement(Mobile from, Point3D p)
        {
            ...
            // 1/16/22, Adam: players trapping players (during PvP) in house previews. Stop it!
            if (toMove != null && res == HousePlacementResult.Valid)
                foreach (object o in toMove)
                    if (o is Mobile m && m != null) // I don't limit it to PlayerMobile since this could be used in PvM as well
