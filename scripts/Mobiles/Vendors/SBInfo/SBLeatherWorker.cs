    ...
    public class SBLeatherWorker : SBInfo
    {
        ...
        public class InternalBuyInfo : ArrayList
        {
            public InternalBuyInfo()
            {
                ...
                {   // balanced buyback support
                    ...
                    Add(new GenericBuyInfo(typeof(Hides), BaseVendor.PlayerPays(typeof(Hides)), 999, 0x1078, 0));

                // 11/1/22, Adam: Why? is it because they are craftable on AI?
                if (Core.RuleSets.ShopkeepersBuyItemsRules())
                    // only sell these on some servers
                    Add(new GenericBuyInfo(typeof(ThighBoots), BaseVendor.PlayerPays(typeof(ThighBoots)), 10, 0x1711, 0));
            }
        }
        ...
        public class InternalSellInfo : GenericSellInfo
        {
            public InternalSellInfo()
            {
                ...
                if (Core.RuleSets.ShopkeepersBuyItemsRules())
                {   // cash buyback
                    Add(typeof(ThighBoots), BaseVendor.VendorPays(typeof(ThighBoots)));
                }
            }
        }
    }
