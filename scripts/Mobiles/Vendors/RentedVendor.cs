...
/* CHANGELOG
 * Scripts/Mobiles/Vendors/RentedVendor.cs
 * 5/24/23, Yoar
 *      Added the following three keywords: "vendor [collect/terminate/contract]"
 *      These options are normally accessible through CMEs, but we don't have CMEs on Siege!
 * 11/16/22, Adam
 *      don't renew this vendor if the house and/or landlord are gone.
 ...
 *	04/16/05, Kitaras
 *		Initial adaptation from runuo 1.0 source to work within a non-aos houseing enviroment
 *		and withen are current houseing/vendor system.	
 */
...
namespace Server.Mobiles
{
    ...
    public class RentedVendor : PlayerVendor
    {
        ...
        public bool Renew
        {
            get { return LandlordRenew && RenterRenew && House != null; }
        }
        ...
        private class RefundOfferPrompt : Prompt
        {
            ...
            public override void OnResponse(Mobile from, string text)
            {
                ...
                else if (Banker.GetAccessibleBalance(from) < amount)
                {
        ...
        // 5/24/23, Yoar: Added speech options because Siege doesn't have context menus
        public override void OnSpeech(SpeechEventArgs e)
        {
            ...
            {
                if (e.HasKeyword(0x3E) || (e.HasKeyword(0x173) && WasNamed(e.Speech))) // vendor collect, *collect
                {
                    if (IsLandlord(from))
                    {
                        if (RentalGold > 0)
                        {
                            int depositedGold = Banker.DepositUpTo(from, RentalGold);
                            RentalGold -= depositedGold;
                ...
                else if (Insensitive.Equals(e.Speech, "vendor terminate") || (Insensitive.EndsWith(e.Speech, " terminate") && WasNamed(e.Speech)))
                {
                    if (IsLandlord(from))
                    {
                        from.SendLocalizedMessage(1062503); // Enter the amount of gold you wish to offer the renter in exchange for immediate termination of this contract?
                        from.Prompt = new RefundOfferPrompt(this);

                        e.Handled = true;
                    }
                }
                else if (Insensitive.Equals(e.Speech, "vendor contract") || (Insensitive.EndsWith(e.Speech, " contract") && WasNamed(e.Speech)))
                {
                    if (IsOwner(from))
                    {
                        from.CloseGump(typeof(RenterVendorRentalGump));
                        from.SendGump(new RenterVendorRentalGump(this));

                        SendRentalExpireMessage(from);
                    ...
                    {
                        from.CloseGump(typeof(LandlordVendorRentalGump));
                        from.SendGump(new LandlordVendorRentalGump(this));
...
#if false
            // 11/16/22, Adam: don't renew this vendor if the house and/or landlord are gone.
            TimeSpan delay = House != null ? m_RentalExpireTime - DateTime.UtcNow : TimeSpan.Zero;
            // abandoning this for now until we resolve vendors disappearing
#else
            if (House == null || Owner == null)
            {
        ...
        private class RentalExpireTimer : Timer
        {
            ...
            {
                m_Vendor.ProcessExpire();
            }
        }

        public void ProcessExpire()
        {
            // 6/27/23, Yoar: Why wasn't this checked previously? Do ALL rented vendors auto-renew?
#if false
            if (!Renew)
            {
                LogHelper log = new LogHelper("VendorExpire.log", false, false);
            ...
            int renewalPrice = this.RenewalPrice;
            int totalGold = this.BankAccount + this.HoldGold;

            //if we dont have the gold at all on vendor destroy is
            // 11/16/22, Adam: don't renew this house of the house and/or landlord are gone.
#if false
            if (renewalPrice > totalGold || this.House == null)
#else
            ...
            {
                if (this.BankAccount >= renewalPrice)
                {
                    this.BankAccount -= renewalPrice;
                    this.RentalGold += renewalPrice;

                    this.RentalPrice = renewalPrice;
                    this.m_RentalExpireTime = DateTime.UtcNow + this.RentalDuration.Duration;
                }
                else
                {
                    this.RentalGold += renewalPrice;
                    renewalPrice -= this.BankAccount;
                    this.BankAccount = 0;

                    this.HoldGold -= renewalPrice;
                    this.RentalPrice = renewalPrice;
                    this.m_RentalExpireTime = DateTime.UtcNow + this.RentalDuration.Duration;
                }
            }
