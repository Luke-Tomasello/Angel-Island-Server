...
/* /Gumps/VendorRentalGumps.cs
 * ChangeLog:
 *  6/26/23, Yoar
 *      Added logging for when a vendor rental contract is voluntarily terminated
 *   4/16/2004, Kitaras
 *	Initial creation/merge from runuo 1.0 source
 *     Adapted and updated to work withen a non aos houseing enviroment
 */
...
namespace Server.Gumps
{
    ...
    public class VendorRentalOfferGump : BaseVendorRentalGump
    {
        ...
        protected override void AcceptOffer(Mobile from)
        {
            ...
            if (!Utility.CanFit(m_Contract.Map, m_Contract.Location, 16, Utility.CanFitFlags.requireSurface))
            {
            ...
            if (price > 0)
            {
                if (Banker.CombinedWithdrawFromAllEnrolled(from, price))
                {
    ...
    public class VendorRentalRefundGump : Gump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (info.ButtonID == 1)
            {
                if (Banker.CombinedWithdrawFromAllEnrolled(m_Landlord, m_RefundAmount))
                {
                    LogHelper log = new LogHelper("VendorExpire.log", false, false);
                    if (m_Vendor.Landlord != null)
                        log.Log(LogType.Mobile, m_Vendor.Landlord, "House owner");
