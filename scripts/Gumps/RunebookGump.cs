 ...
 * ChangeLog
 *  1/19/22, Yoar
 *      Added buttons to rearrange runes without having to drop them (MOVE_RUNES)
 *	02/11/06, Adam
...
#define MOVE_RUNES // Yoar: Rearrange runes
...
namespace Server.Gumps
{
    public class RunebookGump : Gump
    {
        ...
        private static int GroupCount = Enum.GetValues(typeof(ButtonGroup)).Length;

        private static int ToButtonID(ButtonGroup group, int index)
        {
            return 2 + (index * GroupCount) + (int)group;
        }

        private static bool FromButtonID(int buttonID, out ButtonGroup group, out int index)
        {
            if (buttonID >= 2)
            {
                group = (ButtonGroup)((buttonID - 2) % GroupCount);
                index = (buttonID - 2) / GroupCount;
                return true;
            }
        ...
        private void AddIndex()
        {
            ...
            for (int i = 0; i < 16; ++i)
            {
                ...
                // Use charge button
                AddButton(130 + ((i / 8) * 160), 65 + ((i % 8) * 15), 2103, 2104, ToButtonID(ButtonGroup.UseCharge, i), GumpButtonType.Reply, 0);
        ...
        {
            // Use charge button
            AddButton(130 + (half * 160), 65, 2103, 2104, ToButtonID(ButtonGroup.UseCharge, index), GumpButtonType.Reply, 0);
            ...
            if (index < m_Book.Entries.Count)
            {
...
#if MOVE_RUNES
                AddButton(135 + (half * 160), 119, 2437, 2438, ToButtonID(ButtonGroup.DropRune, index), GumpButtonType.Reply, 0);
                AddHtmlLocalized(150 + (half * 160), 115, 100, 18, 1062212, false, false); // Drop

                if (index > 0)
                    AddButton(193 + (half * 160), 116, 5603, 5607, 999 + index, GumpButtonType.Reply, 0); // move left/up

                if (index < m_Book.Entries.Count - 1)
                    AddButton(243 + (half * 160), 116, 5601, 5605, 1000 + index, GumpButtonType.Reply, 0); // move right/down

                AddHtml(213 + (half * 160), 115, 50, 20, "Move", false, false);
#else
                // Drop rune button
                AddButton(135 + (half * 160), 115, 2437, 2438, ToButtonID(ButtonGroup.DropRune, index), GumpButtonType.Reply, 0);
                ...
                if (e != m_Book.Default)
                {
                    ...
                    AddButton(160 + (half * 140), 20, 2361, 2361, ToButtonID(ButtonGroup.SetDefault, index), GumpButtonType.Reply, 0);
                    AddHtmlLocalized(175 + (half * 140), 15, 100, 18, 1011300, false, false); // Set default
                }

                if (Core.RuleSets.AOSRules())
                {
                    AddButton(135 + (half * 160), 140, 2103, 2104, ToButtonID(ButtonGroup.Recall, index), GumpButtonType.Reply, 0);
                    AddHtmlLocalized(150 + (half * 160), 136, 110, 20, 1062722, false, false); // Recall

                    AddButton(135 + (half * 160), 158, 2103, 2104, ToButtonID(ButtonGroup.Gate, index), GumpButtonType.Reply, 0);
                    AddHtmlLocalized(150 + (half * 160), 154, 110, 20, 1062723, false, false); // Gate Travel

                    AddButton(135 + (half * 160), 176, 2103, 2104, ToButtonID(ButtonGroup.SacredJourney, index), GumpButtonType.Reply, 0);
                    AddHtmlLocalized(150 + (half * 160), 172, 110, 20, 1062724, false, false); // Sacred Journey
                }
                else
                {
                    // Recall button
                    AddButton(135 + (half * 160), 140, 2271, 2271, ToButtonID(ButtonGroup.Recall, index), GumpButtonType.Reply, 0);

                    // Gate button
                    AddButton(205 + (half * 160), 140, 2291, 2291, ToButtonID(ButtonGroup.Gate, index), GumpButtonType.Reply, 0);
                }
            }
            else
            {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            else if (buttonID >= 1000)
            {
                ...
                if (m_Book.CheckAccess(from))
                {
                    Switch(m_Book.Entries, index, index + 1);
            ...
            else
            {
                ...
                FromButtonID(buttonID, out group, out index);
                        ...
                        case ButtonGroup.UseCharge:
                            {
                                ...
                                else
                                {
                                    ...
                                    if (Sextant.Format(e.Location, e.Map, ref xLong, ref yLat, ref xMins, ref yMins, ref xEast, ref ySouth))
                                    {
                                        string location = Sextant.Format(xLong, yLat, xMins, yMins, xEast, ySouth);
...
#if RunUO
                        case ButtonGroup.SacredJourney:
                            {
...
#if MOVE_RUNES
        private static void Switch(IList list, int a, int b)
        {
