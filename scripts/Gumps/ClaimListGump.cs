...
/* Scripts/Gumps/ClaimListGump.cs
 * ChangeLog:
 * 5/9/10, Adam
 *	Created
 */
...
namespace Server.Gumps
{
    public class ClaimListGump : Gump
    {
        ...
        public ClaimListGump(AnimalTrainer trainer, Mobile owner, ArrayList list, int page)
            : base(GumpOffsetX, GumpOffsetY)
        {
            owner.CloseGump(typeof(ClaimListGump));
        ...
        public ClaimListGump(AnimalTrainer trainer, Mobile owner, ArrayList list)
            : this(trainer, owner, list, 0)
        {
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            ...
            switch (info.ButtonID)
            {
                ...
                case 1: // Previous
                    {
                        if (m_Page > 0)
                            from.SendGump(new ClaimListGump(m_Trainer, from, m_Mobiles, m_Page - 1));
                ...
                default:
                    {
                        ...
                        if (index >= 0 && index < m_Mobiles.Count)
                        {
                            ...
                            if (m.Deleted)
                            {
                                from.SendMessage("That pet has been deleted.");
                                from.SendGump(new ClaimListGump(m_Trainer, from, m_Mobiles, 0));
                            }
                            else
                            {
                                try
                                {
                                    if (index >= 0 && index < m_Mobiles.Count)
                                        m_Trainer.EndClaimList(from, m_Mobiles[index] as BaseCreature);
                                }
                                catch (Exception ex)
                                {
