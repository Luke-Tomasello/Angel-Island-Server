 ...
 * Changelog:
 * 8/17/22, Adam
 *  Move the GetSharedAccounts family of functions to Utility
 * 6/23/22, Yoar
 *      Rewrote the following pages: Firewall, FirewallInfo, IPException, IPException_IPDetails
 *      More consistent behavior + more informative messages
 * 3/15/16, Adam
...
using System.Net;
using System.Text;
using static Server.CommandSystem;
...
namespace Server.Gumps
{
    ...
    public class AdminGump : Gump
    {
                    ...
                    case AdminGumpPage.Information:
                        {
                            ...
                            AddLabel(150, 170, LabelHue, Firewall.GetAllIPs().Count.ToString());
                    ...
                    case AdminGumpPage.Clients:
                        {
                            ...
                            if (m_List == null)
                            {
                                ...
                                m_List = new(m_List.Cast<NetState>().Where(ns => ns != null && ns.Mobile != null && m_From.Account != null &&
                                    (ns.Mobile.Account as Accounting.Account).AccessLevel <= (m_From.Account as Accounting.Account).AccessLevel
                                ).ToList());
                    ...
                    case AdminGumpPage.Accounts_Shared:
                        {
                            ...
                            if (m_List == null)
                                m_List = Utility.GetAllSharedAccounts();
                    ...
                    case AdminGumpPage.Accounts:
                        {
                            ...
                            if (m_List == null)
                            {
                                ...
                                m_List = new(m_List.Cast<Accounting.Account>().Where(acct => acct != null && m_From.Account != null &&
                                    (m_From.Account as Accounting.Account).AccessLevel >= acct.AccessLevel).ToList());
                                m_List.Sort(Utility.AccountComparer.Instance);
                            }
                    ...
                    case AdminGumpPage.Firewall:
                        {
                            if (m_List == null)
                            {
                                m_List = new ArrayList(Firewall.GetAllIPs());
                                m_List.Sort();
                            }
                            ...
                            for (int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index)
                            {
                                ...
                                AddButton(380, offset - 1, 0xFA5, 0xFA7, GetButtonID(6, index + 3), GumpButtonType.Reply, 0);
                            }
                    ...
                    case AdminGumpPage.FirewallInfo:
                        {
                            ...
                            if (m_List == null)
                            {
                                ...
                                foreach (Account acct in Accounts.Table.Values)
                                {
                                    foreach (IPAddress loginIP in acct.LoginIPs)
                                    {
                                        if (Utility.IPMatch(pattern, loginIP))
                                        {
                                            m_List.Add(acct);
                                            break;
                                        }
                            ...
                            AddLabelCropped(132, 120, 81, 40, LabelHue, "AccessLevel");
                            AddLabelCropped(252, 120, 50, 40, LabelHue, "Status");
                            ...
                            if (m_List.Count == 0)
                            {
                                AddLabel(12, 140, LabelHue, "There are no accounts associated to this IP.");
                                break;
                            }
                            ...
                            for (int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index)
                            {
                                ...
                                AddLabelCropped(12, offset, 120, 20, LabelHue, a.Username);
                                AddLabelCropped(132, offset, 120, 20, LabelHue, FormatAccessLevel(accessLevel));
                                AddLabelCropped(252, offset, 120, 20, online ? GreenHue : RedHue, online ? "Online" : "Offline");
                    ...
                    case AdminGumpPage.IPException:
                        {
                            if (m_List == null)
                            {
                                m_List = new ArrayList(IPException.GetAllIPs());
                                m_List.Sort();
                            }
                            ...
                            if (m_List.Count == 0)
                            {
                                AddLabel(12, 140, LabelHue, "The IP exception list is empty.");
                                break;
                            }
                            ...
                            for (int i = 0, index = (listPage * 12); i < 12 && index >= 0 && index < m_List.Count; ++i, ++index)
                            {
                                ...
                                AddLabelCropped(12, offset, 181, 40, LabelHue, obj.ToString());
                                AddLabelCropped(235, offset, 81, 40, LabelHue, IPException.GetLimit(obj.ToString()).ToString());

                                AddButton(380, offset - 1, 0xFA5, 0xFA7, GetButtonID(2, index + 2), GumpButtonType.Reply, 0);
                            }
        ...
        {
            AddTextField(200, 20, 200, 20, 0);
            AddButtonLabeled(200, 50, GetButtonID(6, 0), "Add (Text)");
            AddButtonLabeled(300, 50, GetButtonID(6, 1), "Add (Targ)");
            AddButtonLabeled(300, 80, GetButtonID(6, 2), "Remove");
        }
        ...
        public void AddIPExceptionHeader()
        {
            ...
            AddButtonLabeled(200, 80, GetButtonID(2, 0), "Add");
            AddButtonLabeled(300, 80, GetButtonID(2, 1), "Remove");
        }
        ...
        public static void BanShared_Callback(Mobile from, bool okay, object state)
        {
            ...
            if (okay)
            {
                ...
                list = Utility.GetSharedAccounts(a.LoginIPs);
        ...
        public static void FirewallShared_Callback(Mobile from, bool okay, object state)
        {
            ...
            if (okay)
            {
                ...
                for (int i = 0; i < a.LoginIPs.Length; ++i)
                    Firewall.Add(a.LoginIPs[i].ToString());
        ...
        public static void Firewall_Callback(Mobile from, bool okay, object state)
        {
            ...
            if (okay)
            {
                Firewall.Add(toFirewall.ToString());
                ...
                case 2: // IPException
                    {
                        switch (index)
                        {
                            case 0: // add
                                {
                                    TextRelay ipRelay = info.GetTextEntry(0);
                                    TextRelay lmRelay = info.GetTextEntry(1);

                                    string ip = (ipRelay == null ? null : ipRelay.Text.Trim());
                                    int lm = (lmRelay == null ? 0 : Utility.ToInt32(lmRelay.Text.Trim()));

                                    if (string.IsNullOrEmpty(ip))
                                    {
                                        from.SendMessage("Invalid IP entered.");
                                    }
                                    else if (lm == 0)
                                    {
                                        from.SendMessage("Invalid limit entered.");
                                    }
                                    else
                                    {
                                        CommandLogging.WriteLine(from, "{0} {1} Adding {2} to IPException with a limit of {3}", from.AccessLevel, CommandLogging.Format(from), ip, lm);
                                        from.SendMessage("Adding {0} to IPException with a limit of {1}.", ip, lm);
                                        IPException.SetLimit(ip, lm);
                                    }
                            ...
                            default: // more info
                                {
                                    ...
                                    if (m_List != null && index >= 0 && index < m_List.Count)
                                    {
                                        ...
                                        if (obj != null)
                                        {
                                            string notice = string.Format("Selected IP address: {0}", obj.ToString());
                                            from.SendGump(new AdminGump(from, AdminGumpPage.IPException_IPDetails, 0, null, notice, obj));
                                        }
                                    }
                ...
                case 3:
                    {
                        ...
                        switch (index)
                        {
                            ...
                            case 100: command = "DocGen"; goto case 108;
                            case 101: command = "TelGen"; goto case 108;
                            case 102: command = "MoonGen"; goto case 108;
                            case 103: command = "UOAMVendors"; goto case 108;
                            case 104: command = "DoorGen"; goto case 108;
                            case 105: command = "SignGen"; goto case 108;
                            case 106: command = "Decorate"; goto case 108;
                            case 107: command = "RebuildCategorization"; goto case 108;
                            case 110: command = "Freeze"; goto case 108;
                            case 120: command = "Unfreeze"; goto case 108;
                            case 108:
                                {   // we no longer allow admins to execute these commands on the production server
                                    if (CheckAccess(command, from.AccessLevel))
                                        switch (index)
                                        {
                                            case 100: InvokeCommand(command); notice = "Documentation has been generated."; break;
                                            case 101: InvokeCommand(command); notice = "Teleporters have been generated."; break;
                                            case 102: InvokeCommand(command); notice = "Moongates have been generated."; break;
                                            case 103: InvokeCommand(command); notice = "Vendor spawners have been generated."; break;
                                            case 104: InvokeCommand(command); notice = "Doors have been generated."; break;
                                            case 105: InvokeCommand(command); notice = "Signs have been generated."; break;
                                            case 106: InvokeCommand(command); notice = "Decoration has been generated."; break;
                                            case 107: InvokeCommand(command); notice = "Categorization menu has been regenerated. The server should be restarted."; break;
                                            case 110: InvokeCommand(command); notice = "Target bounding points."; break;
                                            case 120: InvokeCommand(command); notice = "Target bounding points."; break;
                                        }
                                    else
                                    {
                                        notice = "Access denied."; break;
                                    }
                                    // this break needed for the compiler only. stupid compiler!
                                    break;
                                }


                            case 200: InvokeCommand("Save"); notice = "The world has been saved."; break;
                            case 201: Shutdown(restart: false, save: true); break;
                            case 202: Shutdown(restart: false, save: false); break;
                            case 203: Shutdown(restart: true, save: true); break;
                            case 204: Shutdown(restart: true, save: false); break;
                            case 210:
                            case 211:
                                {
                        ...
                        switch (index)
                        {
                            ...
                            case 111: command = "FreezeWorld"; goto case 123;
                            case 112: command = "FreezeMap"; goto case 123;
                            case 121: command = "UnfreezeWorld"; goto case 123;
                            case 122: command = "UnfreezeMap"; goto case 123;
                            case 123:
                                {   // we no longer allow admins to execute these commands on the production server
                                    if (CheckAccess(command, from.AccessLevel))
                                        switch (index)
                                    ...
                                    {
                                        from.SendMessage("Access denied.");
                                        notice = "Access denied."; break;
                                    }
                                    // this break needed for the compiler only. stupid compiler!
                                    break;
                                    ...
                                    if (match == null || match.Length == 0)
                                    {
                                        try
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                results.Sort(Utility.AccountComparer.Instance);
                                            }
                                        }
                            ...
                            case 16: // view shared
                                {
                                    ...
                                    ArrayList list = Utility.GetSharedAccounts(a.LoginIPs);
                            ...
                            case 0: // add (text)
                                {
                                    ...
                                    {
                                        CommandLogging.WriteLine(from, "{0} {1} Adding {2} to the firewall list", from.AccessLevel, CommandLogging.Format(from), ip);
                                        from.SendMessage("Adding {0} to the firewall list.", ip);
                                        Firewall.Add(ip);
                                    }
                            ...
                            case 2: // remove
                                {
                                    ...
                                    {
                                        from.SendMessage("Removing {0} from the firewall list.", ip);
                                        Firewall.Remove(ip);
                                    }
                            ...
                            default: // more info
                                {
                                    ...
                                    if (m_List != null && index >= 0 && index < m_List.Count)
                                    {
                                        ...
                                        {
                                            string notice = string.Format("Selected IP address: {0}", obj.ToString());
                                            from.SendGump(new AdminGump(from, AdminGumpPage.FirewallInfo, 0, null, notice, obj));
                                        }
                                    }
                ...
                case 9:
                    {
                        if (m_List != null && index >= 0 && index < m_List.Count)
                        {
                            if (m_PageType == AdminGumpPage.AccountDetails_Access_ClientIPs)
                            {
                                ...
                                ArrayList list = Utility.GetSharedAccounts((IPAddress)obj);
