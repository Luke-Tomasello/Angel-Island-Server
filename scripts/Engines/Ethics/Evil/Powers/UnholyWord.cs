...
namespace Server.Ethics.Evil
{
    public sealed class UnholyWord : Power
    {
        ...
        private void Power_OnTarget(Mobile fromMobile, object obj, object state)
        {
            ...
            if (target == null || target.Player == false || !target.Hero)
            {   // not sure
            ...
            DoUnholyWord(from.Mobile, target);

            FinishInvoke(from);
        }

        private void DoUnholyWord(Mobile from, Mobile combatant)
        {
            Mobile[] buf = new Mobile[2] { from, combatant };
            Timer.DelayCall(TimeSpan.FromSeconds(0.5), new TimerStateCallback(DoUnholyWord_Stage1), buf);
        }

        private void DoUnholyWord_Stage1(object state)
        {
            ...
            {
                from.MovingParticles(combatant, 0x36FA, 1, 0, false, true, 1108, 0, 9533, 9534, 0, (EffectLayer)255, 0);
                from.PlaySound(0x1FB);
                Timer.DelayCall(TimeSpan.FromSeconds(1.0), new TimerStateCallback(DoUnholyWord_Stage2), state);
            }
        }

        private void DoUnholyWord_Stage2(object state)
        {
            Mobile from = ((Mobile[])state)[0] as Mobile;
            Mobile combatant = ((Mobile[])state)[1] as Mobile;
