...
namespace Server.Factions
{
    public abstract class BaseFactionGuard : BaseCreature
    {
        ...
        public override void PackStrongPotions(int min, int max)
        {
            PackStrongPotions(Utility.RandomMinMax(min, max));
        }

        public override void PackStrongPotions(int count)
        {
            for (int i = 0; i < count; ++i)
                PackStrongPotion();
        }

        public override void PackStrongPotion()
        {
            PackItem(Loot.Construct(m_StrongPotions));
        }

        public override void PackWeakPotions(int min, int max)
        {
            PackWeakPotions(Utility.RandomMinMax(min, max));
        }

        public override void PackWeakPotions(int count)
        {
            for (int i = 0; i < count; ++i)
                PackWeakPotion();
        }

        public override void PackWeakPotion()
        {
