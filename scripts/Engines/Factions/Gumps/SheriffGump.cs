    ...
    public class SheriffGump : FactionGump
    {
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (index >= 0 && index < m_Town.GuardLists.Count)
                PlaceGuard(m_From, m_Faction, m_Town, m_Town.GuardLists[index]);
        }

        public static void PlaceGuard(Mobile from, Faction faction, Town town, GuardList guardList)
        {
            if (!town.IsSheriff(from) || town.Owner != faction)
            {
            ...
            {
                from.SendMessage("You cannot place a guard here");
            }
            else if (town.Silver >= guardList.Definition.Price)
            {
                ...
                if (guard != null)
                {
                    ...
                    town.Silver -= guardList.Definition.Price;
