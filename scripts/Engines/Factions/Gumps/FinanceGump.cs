...
namespace Server.Factions
{
    public class FinanceGump : FactionGump
    {
        ...
        public static int[] PriceOffsets { get { return m_PriceOffsets; } }
                ...
                case 1: // make vendor
                    {
                        ...
                        if (index >= 0 && index < vendorLists.Count)
                            PlaceVendor(m_From, m_Faction, m_Town, vendorLists[index]);
        ...
        public static void SetTax(Mobile from, Town town, int newTax)
        {
            if (town.Tax == newTax)
                return;

            if (from.AccessLevel == AccessLevel.Player && !town.TaxChangeReady)
            {
                TimeSpan remaining = DateTime.UtcNow - (town.LastTaxChange + FactionConfig.TownTaxChangePeriod);

                if (remaining.TotalMinutes < 4)
                    from.SendLocalizedMessage(1042165); // You must wait a short while before changing prices again.
                else if (remaining.TotalMinutes < 10)
                    from.SendLocalizedMessage(1042166); // You must wait several minutes before changing prices again.
                else if (remaining.TotalHours < 1)
                    from.SendLocalizedMessage(1042167); // You must wait up to an hour before changing prices again.
                else if (remaining.TotalHours < 4)
                    from.SendLocalizedMessage(1042168); // You must wait a few hours before changing prices again.
                else
                    from.SendLocalizedMessage(1042169); // You must wait several hours before changing prices again.
            }
            else
            {
                town.Tax = newTax;
                ...
                if (from.AccessLevel == AccessLevel.Player)
                    town.LastTaxChange = DateTime.UtcNow;
            }
        }

        public static void PlaceVendor(Mobile from, Faction faction, Town town, VendorList vendorList)
        {
            if (!town.IsFinance(from) || town.Owner != faction)
            {
                from.SendLocalizedMessage(1010339); // You no longer control this city
            }
            else if (Town.FromRegion(from.Region) != town)
            {
                from.SendLocalizedMessage(1010305); // You must be in your controlled city to buy Items
            }
            else if (vendorList.Vendors.Count >= vendorList.Definition.Maximum)
            {
                from.SendLocalizedMessage(1010306); // You currently have too many of this enhancement type to place another
            }
            else if (BaseBoat.FindBoatAt(from.Location, from.Map) != null)
            {
                from.SendMessage("You cannot place a vendor here");
            }
            else if (town.Silver >= vendorList.Definition.Price)
            {
                BaseFactionVendor vendor = vendorList.Construct(town, faction);

                if (vendor != null)
                {
                    town.Silver -= vendorList.Definition.Price;
