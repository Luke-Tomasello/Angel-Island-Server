...
namespace Server.Factions
{
    public class FactionStoneGump : FactionGump
    {
                ...
                case 1: // merchant title
                    {
                        if (index >= 0 && index <= MerchantTitles.Info.Length)
                            SetMerchantTitle(m_From, (MerchantTitle)index);
                ...
                case 2: // transfer silver
                    {
                        ...
                        if (index >= 0 && index < towns.Count)
                            TransferSilver(m_From, m_Faction, towns[index], 10000);
        ...
        public static void SetMerchantTitle(Mobile from, MerchantTitle newTitle)
        {
            PlayerState pl = PlayerState.Find(from);
            ...
            if (mti == null)
            {
                from.SendLocalizedMessage(1010120); // Your merchant title has been removed

                if (pl != null)
                    pl.MerchantTitle = newTitle;
            }
            else if (MerchantTitles.IsQualified(from, mti))
            {
                from.SendLocalizedMessage(mti.Assigned);
        ...
        public static void TransferSilver(Mobile from, Faction faction, Town town, int amount)
        {
            if (town.Owner != faction)
            {
                from.SendLocalizedMessage(1010332); // Your faction does not control this town
            }
            else if (faction.Silver < amount)
            {
                from.SendLocalizedMessage(1042204); // You do not have enough silver.
            }
            else
            {
                faction.Silver -= amount;
                town.Silver += amount;

                if (amount == 10000)
                    from.SendLocalizedMessage(1042726, true, " " + town.Definition.FriendlyName); // 10k in silver has been received by:
                else
                    from.SendMessage("{0}k in silver has been received by: {1}", amount / 1000, town.Definition.FriendlyName);
            }
        }
    }
}