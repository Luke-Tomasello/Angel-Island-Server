 ...
 * CHANGELOG:
 *  5/5/23, Adam (CanTalk())
 *      Check if the player is in a BadRegion (Prison or Jail), if so, issue the message
 *          "You may not use chat while incarcerated."
 *	7/8/08, Adam
 *		PrivateMessage was throwing an exception if there was no 'text'. This is because param.IndexOf(' ') was returning -1, and we were using that 
 *		value as input into Substring. I.e., param.Substring(0, indexOf);
 */
...
namespace Server.Engines.Chat
{
    public class ChatActionHandlers
    {
...
#else
            Register(0x43, false, true, new OnChatAction(LeaveChannel));
            Register(0x61, false, true, new OnChatAction(ChannelMessage));
            Register(0x62, false, false, new OnChatAction(JoinChannel));
            Register(0x63, false, false, new OnChatAction(CreateChannel));
#endif
        }
        ...
        public static void ChannelMessage(ChatUser from, Channel channel, string param)
        {
            ...
            {
                if (channel.BadRegion(from))
                    from.Mobile.SendMessage("You may not use chat while incarcerated.");
                else
                    from.SendMessage(36); // The moderator of this conference has not given you speaking priviledges.
            }
        }
