 ...
 * CHANGELOG:
 *  5/5/23, Adam (CanTalk())
 *      Check if the player is in a BadRegion (Prison or Jail)
 */
...
namespace Server.Engines.Chat
{
    public class Channel
    {
        ...
        {
            return ((!m_VoiceRestricted || m_Voices.Contains(user) || m_Moderators.Contains(user)) && !BadRegion(user));
        }

        public bool BadRegion(ChatUser user)
        {
            if (user.Mobile.Region != null && user.Mobile.Region.IsAngelIslandRules || user.Mobile.Region.IsJailRules)
                return true;
            return false;
        }
        ...
        public void AddBan(ChatUser user)
        {
            AddBan(user, null);
        }
