...
/* Scripts/Engines/Pathing/FastAStarAlgorithem.cs
 * * ChangeLog
 * 	12/02/05, erlein
 *		Also changed back the search depth and re-set area size to 38 (original).
 *	12/02/05, erlein
 *		Changed it back again to prevent mass CPU usage.
 *  11/29/05, Kit
 *		Changed area size from 32 to 256 for use with NavStar System
 */
...
namespace Server.PathAlgorithms.FastAStar
{
    ...
    public class FastAStarAlgorithm : PathAlgorithm
    {
        ...
        {
            //Diagnostics.LogHelper logger = new Diagnostics.LogHelper("AI.log", false);
            try
            {
                ...
                //logger.Log(Diagnostics.LogType.Text,string.Format("{0}:{1}:{2}:{3}", m_Nodes[m_OpenList].cost, m_Nodes[m_OpenList].total, m_Nodes[m_OpenList].parent, m_Nodes[m_OpenList].z));
                ...
                while (m_OpenList != -1 && CheckOpenList())
                {
                    OuterLoop++;
                    ...
                    int count = GetSuccessors(bestNode, obj_start);
                    ...
                    for (int i = 0; i < count; ++i)
                    {
                        ...
                        if (!wasTouched)
                        {
                            ...
                            if (!wasTouched || m_Nodes[newNode].total > newTotal)
                            {
                                ...
                                //logger.Log(Diagnostics.LogType.Text, string.Format("{0}:{1}:{2}:{3}", m_Nodes[newNode].cost, m_Nodes[newNode].total, m_Nodes[newNode].parent, 0));
        ...
        public int GetSuccessors(int p, Movement.MovementObject obj_start)
        {
            int px = p % AreaSize;
            ...
            for (int i = 0; i < 8; ++i)
            {
                ...
                if (CalcMoves.CheckMovement(obj_start, p3D, (Direction)i, out z))
                {
