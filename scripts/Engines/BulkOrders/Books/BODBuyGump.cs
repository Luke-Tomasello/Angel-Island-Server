...
namespace Server.Engines.BulkOrders
{
    public class BODBuyGump : Gump
    {
                ...
                if (m_Book.Entries.Contains(m_Object) && pv != null)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if ((pack != null && pack.ConsumeTotal(typeof(Gold), price)) || Banker.CombinedWithdrawFromAllEnrolled(m_From, price))
                            {
        ...
        public BODBuyGump(PlayerMobile from, BulkOrderBook book, object obj, int price)
            : base(100, 200)
        {
            m_From = from;
            m_Book = book;
