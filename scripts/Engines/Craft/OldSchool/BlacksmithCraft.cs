...
/* Scripts\Engines\Craft\OldSchool\BlacksmithCraft.cs
 * ChangeLog:
 *	06/21/11, Adam
...
{
    public class BlacksmithCraft : SkillCraft
    {
        #region Constants
        public enum Context
        {
            TopMenu,            //	i.e., Repair, Smelt Shields, Armor, Weapons
        ...
        public enum ShieldMenuSelectionID
        {
        ...
        public BlacksmithCraft(OldSchoolCraft craft)
            : base(craft)
        {
        }
        ...
        public void ShowTopMenu()
        {
            ...
            //	i.e., Repair, Smelt Shields, Armor, Weapons
            List<Selection> temp = new List<Selection>();
            temp.Add(new Selection(new ItemListEntry("Repair", (int)TopMenuSelectionID.Repair, 0), (int)Context.TopMenu));
            temp.Add(new Selection(new ItemListEntry("Smelt", (int)TopMenuSelectionID.Smelt, 0), (int)Context.TopMenu));
            temp.Add(new Selection(new ItemListEntry("Shields", (int)TopMenuSelectionID.Shields, 0), (int)Context.TopMenu));
            temp.Add(new Selection(new ItemListEntry("Armor", (int)TopMenuSelectionID.Armor, 0), (int)Context.TopMenu));
            temp.Add(new Selection(new ItemListEntry("Weapons", (int)TopMenuSelectionID.Weapons, 0), (int)Context.TopMenu));
            ...
            Craft.From.SendMenu(new ItemMenu(this, bsOptions, "Blacksmithy Options"));
        }
        ...
        public void HandleTopMenu(int operation)
        {
            ...
            {
                case TopMenuSelectionID.Repair:
                    prompt = 1044276;   // Target an item to repair.
                    break;
                case TopMenuSelectionID.Smelt:
                    prompt = 1044273;   // Target an item to recycle.
                    break;
                case TopMenuSelectionID.Armor:
                case TopMenuSelectionID.Shields:
                case TopMenuSelectionID.Weapons:
        ...
        #region Shields
        private void DoShieldsClassMenu(object targeted)
        {
            // there aren't any different shied classes, so jump directly to the list of shields
            // 0x1b72 is simply 
            //ShowClassMenu(Shileds, 1011080, 1011080, Context.PickShieldClass);
            HandleShieldsClassMenu((int)ShieldMenuSelectionID.Shields);
        }
        public void HandleShieldsClassMenu(int operation)
        {
            int group = 0;
            switch ((ShieldMenuSelectionID)operation)
            {
                case ShieldMenuSelectionID.Shields:
                    group = 1011080;
                    break;
            }

            ShowItemMenu(group, Shields, Context.PickShieldPiece);
        }
        public void HandlePickShieldsPiece(int operation)
        {
            MakeItem(operation, Shields);
        }
        ...
        private itemTable[] GetGroupTable(int index)
        {
            switch (index)
            {
                case 1011080: return Shields;       // Shields
        ...
        private int GetGroupImage(int index)
        {
            switch (index)
            {
                case 1011080: return 0x1b72;        //	"Shields";
        ...
        public override void OnMenuSelection(int context, int operation)
        {
            ...
            {
                case BlacksmithCraft.Context.TopMenu:
                    HandleTopMenu(operation);
                    break;

                case BlacksmithCraft.Context.PickArmorClass:
                    HandleArmorClassMenu(operation);
                    break;

                case BlacksmithCraft.Context.PickArmorPiece:
                    HandlePickArmorPiece(operation);
                    break;

                case BlacksmithCraft.Context.PickShieldClass:
                    HandleShieldsClassMenu(operation);
                    break;

                case BlacksmithCraft.Context.PickShieldPiece:
                    HandlePickShieldsPiece(operation);
                    break;

                case BlacksmithCraft.Context.PickWeaponClass:
                    HandleWeaponsClassMenu(operation);
                    break;

                case BlacksmithCraft.Context.PickWeaponPiece:
            ...
            else
            {
                switch ((Context)context)
                {
                    case Context.PickResource:
                        switch ((TopMenuSelectionID)operation)
                        {
                            case TopMenuSelectionID.Repair: DoRepair(targeted); return;
                            case TopMenuSelectionID.Smelt: DoSmelt(targeted); return;
                            case TopMenuSelectionID.Armor: DoArmorClassMenu(targeted); return;
                            case TopMenuSelectionID.Shields: DoShieldsClassMenu(targeted); return;
                            case TopMenuSelectionID.Weapons: DoWeaponsClassMenu(targeted); return;
                        }
                        break;
                }
            }
        ...
        /// </summary>
        private static itemTable[] Shields = new itemTable[]
        {
			// Shields
			itemTable.AddCraft(typeof(Buckler), 1011080, 1027027, -25.0, 25.0, typeof(IronIngot), 1044036, 10, 1044037),
            itemTable.AddCraft(typeof(BronzeShield), 1011080, 1027026, -15.2, 34.8, typeof(IronIngot), 1044036, 12, 1044037),
            itemTable.AddCraft(typeof(HeaterShield), 1011080, 1027030, 24.3, 74.3, typeof(IronIngot), 1044036, 18, 1044037),
            itemTable.AddCraft(typeof(MetalShield), 1011080, 1027035, -10.2, 39.8, typeof(IronIngot), 1044036, 14, 1044037),
            itemTable.AddCraft(typeof(MetalKiteShield), 1011080, 1027028, 4.6, 54.6, typeof(IronIngot), 1044036, 16, 1044037),
            itemTable.AddCraft(typeof(WoodenKiteShield), 1011080, 1027032, -15.2, 34.8, typeof(IronIngot), 1044036, 8, 1044037)
        };
