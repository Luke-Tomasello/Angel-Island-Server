...
/* Engines/Crafting/Core/Repair.cs
 * CHANGELOG:
 *  10/24/23, Yoar
 *      Merge with RunUO 2.0
 *      Added support for repair deeds
 *  11/23/21, Yoar
 *      Added call to DefTownshipCraft
 *  11/22/21, Yoar
 *      Changed type of "message" from int to string in order to support string notices.
 *  11/9/08, Adam
 *      - merged in RunUO 2.0 code.
 *      - stub out repair contracts
 *  03/09/07, plasma
 *      Fixed repair chance for armour and weaps
 *  6/5/04, Pix
...
namespace Server.Engines.Craft
{
    public class Repair
    {
        public static bool RepairDeedsEnabled { get { return Core.RuleSets.SiegeStyleRules(); } }
        ...
        private class InternalTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (m_Tool != null && m_CraftSystem.CanCraft(from, m_Tool, targeted.GetType()) == 1044267)
                {
                    number = 1044282; // You must be near a forge and and anvil to repair items. * Yes, there are two and's *
                }
                else if (m_CraftSystem is Server.Township.DefTownshipCraft)
                {
                    number = Server.Township.DefTownshipCraft.ProcessRepair(from, targeted, usingDeed, ref toDelete);
                }
                else if (m_CraftSystem is DefTinkering && targeted is Golem)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (!from.CheckSkill(SkillName.Tinkering, 0.0, 100.0, contextObj: new object[2]))
                                damage /= 2;
                ...
                else if (targeted is BaseWeapon)
                {
                    ...
                    else if (!Core.RuleSets.AOSRules() && weapon.PoisonCharges != 0)
                    {
                ...
                else if (RepairDeedsEnabled && !usingDeed && targeted is BlankScroll)
                {
                    SkillName skill = m_CraftSystem.MainSkill;
