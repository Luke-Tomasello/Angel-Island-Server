...
/* Engines/Crafting/DefBlacksmithy.cs
 * CHANGELOG:
 *  9/26/23, Yoar
 *      Enabled crafting animation (old-school)
 *  12/16/21, Yoar
 ...
 *	3/18/08 - Pix
 *		removed dragonscale stuff (again)
 *  10/26/05 Taran Kain
 *		Increased PlateArms maxskill from 116.0 to 116.3 (to follow with standard 50-pt spread)
 *		Lowered PlateChest's skills from 75-125 to 71.2-121.2, to raise exceptional rate slightly
 *  6/5/04, Pix
...
namespace Server.Engines.Craft
{
    public class DefBlacksmithy : CraftSystem
    {
            ...
            : base(1, 1, 1.25) // base( 1, 2, 1.7 )
        {
			...
			*/
            ...
            if (Core.RuleSets.StandardShardRules())
            {
                MinCraftEffect = 2;
                MaxCraftEffect = 3;
                Delay = 2.2;        // this deley seems to match UOSA pretty well
            }
        }
        ...
        public static bool HasAnvil(Mobile from, int range)
        {
            return HasAnvilAndForge(from, range, true, false);
        }

        public static bool HasForge(Mobile from, int range)
        {
            return HasAnvilAndForge(from, range, false, true);
        }

        public static bool HasAnvilAndForge(Mobile from, int range)
        {
            return HasAnvilAndForge(from, range, true, true);
        }

        private static bool HasAnvilAndForge(Mobile from, int range, bool needsAnvil, bool needsForge)
        {
            bool anvil, forge;

            CheckAnvilAndForge(from, range, out anvil, out forge);

            return ((!needsAnvil || anvil) && (!needsForge || forge));
        }
        ...
        public static void CheckAnvilAndForge(Mobile from, int range, out bool anvil, out bool forge)
        {
            ...
            {
                #region Cooperatives
                Multis.BaseHouse house = Multis.BaseHouse.FindHouseAt(item);

                if (house != null && house.MembershipOnly && !house.IsMember(from))
                    continue; // cannot use an anvil or forge of a members-only mining cooperative
                #endregion
        ...
        public override TextDefinition CanCraft(Mobile from, BaseTool tool, Type itemType)
        {
            ...
            else if (!HasAnvilAndForge(from, 2))
        ...
        public override void InitCraftList()
        {
			...
			*/
            ...
            if (BaseScales.Enabled)
            {
#if false
                // Dragon Scale Armor
                int index = AddCraft(typeof(DragonGloves), 1053114, 1029795, 68.9, 118.9, typeof(RedScales), 1060883, 16, 1060884);
                ...
                SetUseSubRes2(index, true);
#else
                const string dragonPlateArmor = "Dragon Plate Armor";

                int index = AddCraft(typeof(DragonPlateArms), dragonPlateArmor, "dragon plate arms", 76.3, 126.3, typeof(RedScales), 1060883, 24, 1060884);
                SetUseSubRes2(index, true);

                index = AddCraft(typeof(DragonPlateGloves), dragonPlateArmor, "dragon plate gloves", 68.9, 118.9, typeof(RedScales), 1060883, 16, 1060884);
                SetUseSubRes2(index, true);

                index = AddCraft(typeof(DragonPlateGorget), dragonPlateArmor, "dragon plate gorget", 66.4, 116.4, typeof(RedScales), 1060883, 14, 1060884);
                SetUseSubRes2(index, true);

                index = AddCraft(typeof(DragonPlateHelm), dragonPlateArmor, "dragon plate helm", 72.6, 122.6, typeof(RedScales), 1060883, 20, 1060884);
                SetUseSubRes2(index, true);

                index = AddCraft(typeof(DragonPlateLegs), dragonPlateArmor, "dragon plate legs", 78.8, 128.8, typeof(RedScales), 1060883, 28, 1060884);
                SetUseSubRes2(index, true);

                index = AddCraft(typeof(DragonPlateChest), dragonPlateArmor, "dragon plate chest", 85.0, 135.0, typeof(RedScales), 1060883, 36, 1060884);
                SetUseSubRes2(index, true);
#endif
            }
