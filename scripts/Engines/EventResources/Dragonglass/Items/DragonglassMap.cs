...
/* Scripts\Engines\EventResources\Dragonglass\Items\DragonglassMap.cs
 * CHANGELOG:
 *  4/26/2024, Adam
 *      Initial version.
 */
...
{
    public class DragonglassMap : ResourceMap
    {
        public override HarvestDefinition HarvestDefinition { get { return Mining.System.OreAndStone; } }

        [Constructable]
        public DragonglassMap()
            : this(8/*tabby says 8*/)
        {
        }

        [Constructable]
        public DragonglassMap(int uses)
            : base(CraftResource.Dragonglass, uses, Map.Trammel)
        {
        }
        ...
        protected override Point2D GetRandomLocation()
        {
            ...
            int tries = 0;
            Point2D loc = Point2D.Zero;
            do
            {
                loc = m_Locations[Utility.Random(m_Locations.Length)];
            } while (BadLocation(loc) && tries++ < 50);
        ...
        {
            Rectangle2D rect = new Rectangle2D(width: 8, height: 8, center: loc);
            List<Point3D> points = rect.PointsInRect3D(map: BankMap);
            foreach (Point3D p in points)
            {
                LandTarget land = new LandTarget(p, BankMap);
                if (land != null)
                {
        ...
        public DragonglassMap(Serial serial)
            : base(serial)
        {
        }
