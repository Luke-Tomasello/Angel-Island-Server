...
/* Scripts/Engines/Party/Party.cs
 * Changelog:
 *  4/15/23, Yoar
 *      Added support for new Alignment system.
 *  8/28/22, Yoar
 *      You can no longer party members of the opposing faction.
 *	07/23/08, weaver
 *		Automated IPooledEnumerable optimizations. 1 loops updated.
 *	01/09/06 Taran Kain
 *		Added Reporting calls
 *  6/5/04, Pix
...
namespace Server.Engines.PartySystem
{
    public class Party : IParty
    {
        ...
        public static void Initialize()
        {
            ...
            Server.CommandSystem.Register("ListenToParty", AccessLevel.GameMaster, new CommandEventHandler(ListenToParty_OnCommand));
        }
        ...
        public void OnAccept(Mobile from, bool force)
        {
            ...
            #region Alignment
            if (AlignmentSystem.Enabled && AlignmentSystem.IsEnemy(m_Leader, from) && !force)
                return;
            #endregion
        ...
        public static void Invite(Mobile from, Mobile target)
        {
            ...
            #region Alignment
            if (AlignmentSystem.Enabled && AlignmentSystem.IsEnemy(from, target))
            {
                from.SendMessage("You cannot have players from opposing alignment in the same party!");
                target.SendMessage("The party cannot have members from opposing alignment.");
            }
            #endregion

            //if (target.AccessLevel > AccessLevel.Player && from.AccessLevel == AccessLevel.Player)
            //{
            //    from.SendMessage("You cannot add staff members to a party.");
            //    return;
            //}
        ...
        private void SendToStaffMessage(Mobile from, string text)
        {
            ...
            if (true)
            {
                string filename = Utility.ValidFileName(string.Format("{0}", from)) + ".log";
