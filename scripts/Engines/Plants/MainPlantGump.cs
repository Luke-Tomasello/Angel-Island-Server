...
/* Scripts\Engines\Plants\MainPlantGump.cs
 * CHANGELOG:
 *  10/30/23, Yoar
 *      QOL Change: Automatically find a water container to pour over the plant when you press the water button.
 *      If it does not find a water container, do the target as usual.
 *  3/26/22, Adam
 *      When we have a tree and it's stage 5(?) we show a sapling as it's the closest thing to a tree we can display.
 *      I.e., as a test I tried to display a date palm which is a single item, and even though it displays, it exceeds the display window by a lot.
 *      at this time we will punt and simply use the 'sapling' graphic for all 'trees' of this age.
 *  3/24/22, Adam
 *      Add support for trees
 *      We still need Yoar's addon painting code to display the correct graphic
 */
    ...
    public class MainPlantGump : Gump
    {
        ...
        private void DrawPlant()
        {
            ...
            else
            {
                ...
                if (TreeSeed.IsTree(m_Plant.PlantType))
                    itemID = 0xCE9; // sapling
                else
                    itemID = typeInfo.ItemID;

                AddItem(130 + typeInfo.OffsetX, 96 + typeInfo.OffsetY, itemID, hueInfo.Hue);
            }
...
#else
                        // 10/30/23, Yoar: QOL change. Does OSI not do this?
                        AddWater(from);
#endif
        ...
        private void AddWater(Mobile from)
        {
            Item item = GetWater(from);
        ...
        public static Item GetWater(Mobile from)
        {
            if (from.Backpack == null)
                return null;

            Item[] items = from.Backpack.FindItemsByType(new Type[] { typeof(BaseBeverage) });
