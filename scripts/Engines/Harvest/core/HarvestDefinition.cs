...
/* scripts/engines/harvest/core/HarvestDefinition.cs
 * Changelog:
 * 
 * 11/17/21, Yoar
 *      Added HarvestDefinition.RandomizeVeins
 * 7/26/21, Pix
 *      Changed seeding of random to use CoreAI.HarvestConstantX (and Y, M) instead of hardcoded values.
 */
...
namespace Server.Engines.Harvest
{
    public class HarvestDefinition
    {
        ...
        public HarvestVein GetVeinAt(Map map, int x, int y)
        {
            ...
            {
                //PIX: random being uses with a deterministic number lets us not have to save
                // what resource is being harvested at that point.  We've externalized the constants
                // that came with RunUO so that 1) we can set these to different numbers so that
                // people coming from RunUO shards don't already know where all the special ores/resources are
                // and 2) so we can change the locations when we want to
                int xconst = CoreAI.HarvestConstantX; //17
                int yconst = CoreAI.HarvestConstantY; //11
                int mconst = CoreAI.HarvestConstantM; //3

                Random random = new Random((x * xconst) + (y * yconst) + (map.MapID * mconst));
