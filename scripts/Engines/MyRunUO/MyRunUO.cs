...
/* Scripts/Engines/MyRunUO/MyRunUO.cs
 * Changelog:
 *  04/28/05 TK
 *		Fixed a couple SQL statements that were causing exceptions when used with
 *		transactions and InnoDB tables.
 *  6/5/04, Pix
...
namespace Server.Engines.MyRunUO
{
    ...
    public class MyRunUO : Timer
    {
        ...
        public void CollectMobiles(DateTime endTime)
        {
            ...
            {
                if (m_Index == 0)
                    m_Collecting.AddRange(Accounts.Table.Values);

                for (int i = m_Index; i < m_Collecting.Count; ++i)
                {
                    Account acct = (Account)m_Collecting[i];
        ...
        public void InsertMobile(Mobile mob)
        {
            ...
            if (Config.LoadDataInFile)
            {
                ...
                m_OpMobiles.Write(EntrySep);
                m_OpMobiles.Write(mob.LongTermMurders);
                m_OpMobiles.Write(EntrySep);
...
#else
        // from RUNUO 2.0. Can delete #ifdef once this is proven out
