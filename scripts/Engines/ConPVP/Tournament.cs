...
/* Scripts\Engines\ConPVP\Tournament.cs
 * Changelog:
 *  3/23/22, Yoar
 *      - Now properly keeping a ref to the Slice timer and stopping that timer whne the TournamentController is deleted.
 *      - Disabled cash prizes.
 *      - Added logging of winners.
 *  3/14/22, Yoar
...
namespace Server.Engines.ConPVP
{
                        ...
                        case TournamentStage.Signup:
                            {
                                ...
                                else if (TournamentController.IsParticipating(from))
                                {
                                    if (m_Registrar != null)
                                        m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                                            0x22, false, "You have already entered another tournament.", from.NetState);
                                }
                                else
                                {
                ...
                for (int i = 0; i < opts.Length; ++i)
                {
                    if (defs[i] != opts[i])
                    {
                        ...
                        if (name != null) // sanity
                        {
                            ...
                            AddBorderedText(60, y, 215, 22, name, LabelColor32, BlackColor32);
                        }
                    ...
                    case TournamentStage.Signup:
                        {
                            ...
                            for (int i = 0; i < m_Players.Count; ++i)
                            {
                                ...
                                else if (TournamentController.IsParticipating(mob))
                                {
                                    if (m_Registrar != null)
                                    {
                                        if (mob == from)
                                        {
                                            m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                                                0x35, false, "You have already entered another tournament.", from.NetState);
                                        }
                                        else
                                        {
                                            m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                                                0x35, false, string.Format("{0} has already entered another tournament.", mob.Name), from.NetState);
                                        }
                                    }
        ...
        private void AddPlayer_OnTarget(Mobile from, object obj)
        {
            ...
            else
            {
                ...
                else if (TournamentController.IsParticipating(mob))
                {
                    ...
                    if (m_Registrar != null)
                        m_Registrar.PrivateOverheadMessage(MessageType.Regular,
                            0x22, false, "They have already entered another tournament.", from.NetState);
                }
                else if (m_Players.Count >= m_Tournament.PlayersPerParticipant)
                {
                    m_From.SendGump(new ConfirmSignupGump(m_From, m_Registrar, m_Tournament, m_Players));
    ...
    public class TournamentController : Item
    {
        ...
        private static bool m_PreventDuels = false;

        public static bool PreventDuels
        {
            get { return m_PreventDuels; }
            set { m_PreventDuels = value; }
        }
        ...
        public static bool IsParticipating(Mobile m)
        {
            for (int i = 0; i < m_Instances.Count; ++i)
            {
                TournamentController controller = (TournamentController)m_Instances[i];

                if (controller != null && !controller.Deleted && controller.Tournament != null && controller.Tournament.Stage != TournamentStage.Inactive && controller.Tournament.HasParticipant(m))
                    return true;
            }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (from.AccessLevel >= AccessLevel.GameMaster && m_Tournament != null)
            {
                ...
                from.SendGump(new PickRulesetGump(from, null, m_Tournament.Ruleset, m_Tournament.OnRulesetUpdated));
            }
        }
    ...
    public class Tournament
    {
        ...
        private bool m_SignupAutoExtend;
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool SignupAutoExtend
        {
            get { return m_SignupAutoExtend; }
            set { m_SignupAutoExtend = value; }
        }
        ...
        public void Serialize(GenericWriter writer)
        {
            ...
            writer.Write((bool)m_SignupAutoExtend);

            writer.Write((TimeSpan)m_FightDelay);
        ...
        public Tournament(GenericReader reader)
        {
            ...
            switch (version)
            {
                case 6:
                    {
                        m_SignupAutoExtend = reader.ReadBool();

                        goto case 5;
                    }
                case 5:
                    {
                        m_FightDelay = reader.ReadTimeSpan();
                ...
                case 1:
                    {
                        ...
                        m_SuddenDeath = reader.ReadTimeSpan();
                ...
                case 0:
                    {
                        ...
                        if (version < 5)
                            m_FightDelay = TimeSpan.FromSeconds(30);

                        if (version < 6)
                            m_SignupAutoExtend = true;
        ...
        public void OnRulesetUpdated(Mobile from, DuelContext context, Ruleset ruleset)
        {
            int count = 0;

            if (m_Pyramid.Levels.Count > 0)
            {
                ...
                for (int i = 0; i < activeLevel.Matches.Count; ++i)
                {
                    ...
                    if (match.InProgress)
                    {
                        DuelContext dc = match.Context;

                        dc.Ruleset.Options.SetAll(false);
                        dc.Ruleset.Options.Or(m_Ruleset.Options);
            ...
            if (count > 0)
                from.SendMessage("Applied new ruleset to {0} running match{1}.", count, count == 1 ? "" : "es");
        }
        ...
        private void GiveAwards(ArrayList players, TrophyRank rank, int cash)
        {
            ...
            for (int i = 0; i < players.Count; ++i)
            {
                ...
                Log("Player {0}, in {1}, has won {2} rank.", mob, title, rank);
                ...
                // Yoar: let's not give out cash prizes
#if false
                if (cash > 0)
                {
        ...
        public void StartTimer()
        {
            ...
            m_Timer = Timer.DelayCall(SliceInterval, SliceInterval, new TimerCallback(Slice));
        }
        ...
        public void Slice()
        {
                ...
                if (until <= TimeSpan.Zero)
                {
                    for (int i = m_Participants.Count - 1; i >= 0; --i)
                    {
                        ...
                        for (int j = 0; j < part.Players.Count; ++j)
                        {
                            ...
                            if (check.Deleted || check.Map == null || check.Map == Map.Internal || !check.Alive || Factions.Sigil.ExistsOn(check) || Engines.Alignment.TheFlag.ExistsOn(check) || check.Region.IsJailRules)
                            {
                                bad = true;
                                break;
                            }
                        ...
                        // TODO: Make a cancel command or gump like this?
                    ...
                    else if (m_SignupAutoExtend)
                    {
                        Alert("Is this all?", "Signup extended.");
                        m_SignupStart = DateTime.UtcNow;
                    }
                    else
                    {
                        Alert("Signup has ended, and not enough participants signed up.", "The tournament has been cancelled.");
                        m_Stage = TournamentStage.Inactive;
                    }
                }
            ...
            else if (m_Stage == TournamentStage.Fighting)
            {
                if (m_Undefeated.Count == 1)
                {
                    ...
                    try
                    {
                        if (m_EventController != null && m_EventController.HasTeamNames)
                ...
                else if (m_Pyramid.Levels.Count > 0)
                {
                    ...
                    for (int i = 0; i < activeLevel.Matches.Count; ++i)
                    {
                        ...
                        if (match.Winner == null)
                        {
                            ...
                            if (!match.InProgress && CanStartMatch(match))
                            {
        ...
        private bool CanStartMatch(TournyMatch match)
        {
            foreach (TournyParticipant tournyPart in match.Participants)
            {
                if (tournyPart.LastMatch + m_FightDelay > DateTime.UtcNow)
                    return false;
            }
        ...
        {
            string toLog = string.Format(format, args);

            LogHelper Logger = new LogHelper("tournament.log", false, true);
            Logger.Log(LogType.Text, toLog);
            Logger.Finish();

            // TODO: Write to in-game book?
        }
    }
                ...
                case TournyType.RandomTeam:
                    {
                        ...
                        if (groupType == GroupingType.Random)
    ...
    public class TournyParticipant : IComparable
    {
        ...
        {
            get { return m_LastMatch; }
            set { m_LastMatch = value; }
        }
                        ...
                        if (changes > 0)
                        {
                            ...
                            for (int i = 0; i < opts.Length; ++i)
                            {
                                if (defs[i] != opts[i])
                                {
                                    ...
                                    {
                                        AddImage(35, y, opts[i] ? 0xD3 : 0xD2);
                                        AddHtml(60, y, 215, 22, name, false, false);
                                    }
                ...
                case TournyBracketGumpType.Round_Info:
                    {
                        ...
                        for (int i = 0; i < count; ++i, y += 18)
                        {
                            ...
                            else if ((m_Tournament.EventController != null && m_Tournament.EventController.HasTeamNames) || m_Tournament.TournyType == TournyType.RandomTeam || m_Tournament.TournyType == TournyType.RedVsBlue)
                            {
                                for (int j = 0; j < match.Participants.Count; ++j)
                                {
                                    if (sb.Length > 0)
                                    ...
                                    if (m_Tournament.EventController != null && m_Tournament.EventController.HasTeamNames)
    ...
    public class TournamentBracketItem : Item
    {
        ...
        public override void OnDoubleClick(Mobile from)
        {
            if (!from.InRange(GetWorldLocation(), 2) && from.AccessLevel == AccessLevel.Player)
            {
                from.LocalOverheadMessage(MessageType.Regular, 0x3B2, 1019045); // I can't reach that
            }
            else if (m_Tournament != null)
