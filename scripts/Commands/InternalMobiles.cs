...
namespace Server.Gumps
{
    public class InternalMobilesGump : Gump
    {
        ...
        {
            CommandSystem.Register("InternalMobiles", AccessLevel.Administrator, new CommandEventHandler(InternalMobiles_OnCommand));
        }
        ...
        public InternalMobilesGump(Mobile owner, ArrayList list, int page)
        ...
        public void Initialize(int page)
        {
            ...
            for (int i = 0, index = page * EntryCount; i < EntryCount && index < m_Names.Count; ++i, ++index)
            {
                ...
                AddLabelCropped(x + TextOffsetX, y, EntryWidth - TextOffsetX, EntryHeight, item is BaseCreature && ((BaseCreature)item).IsAnyStabled ? 33 : 0x58 /*hue*/,
                    item.Deleted ? "(deleted)" : (string.Format("{0}:{1}:{2}:{3} Stabled:{4}", item.GetType().Name, item is PlayerMobile ? ((Account)item.Account).Username : "Not Player", item.Map, item.Location, item is BaseCreature && ((BaseCreature)item).IsAnyStabled)));
            ...
            public override void OnResponse(Server.Network.NetState sender, RelayInfo info)
            {
                if (info.ButtonID == 2)
                {
                    foreach (Mobile item in m_Names)
                    {
                        ...
                        {
                            if (((BaseCreature)item).IsAnyStabled == false)
                                item.Delete();
                        }
                        else
                        {
