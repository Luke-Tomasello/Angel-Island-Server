...
namespace Server.Commands
{
    public class Docs
    {
        ...
        {
            Server.CommandSystem.Register("DocGen", AccessLevel.Owner, new CommandEventHandler(DocGen_OnCommand));
        }
        ...
        private static void GenerateStyles()
        {
            using (StreamWriter css = GetWriter("docs/", "styles.css"))
            {
                ...
                css.WriteLine("td.sp {{ background-color: #{0:X6}; }}", Core.RuleSets.AOSRules() ? SpinedAOS : SpinedLBR);
                css.WriteLine("td.ho {{ background-color: #{0:X6}; }}", Core.RuleSets.AOSRules() ? HornedAOS : HornedLBR);
                css.WriteLine("td.ba {{ background-color: #{0:X6}; }}", Core.RuleSets.AOSRules() ? BarbedAOS : BarbedLBR);
            }
        }
        ...
        private static void DocumentBulkOrders()
        {
            ...
            {
                BulkOrderSystem system = DefSmith.System;
                ...
                DocumentSmithBOD(html, system.GetAllRewards(system.ComputePoints(sbod)), "10, 15, 20: Normal", sbod.Material);
                sbod.RequireExceptional = true;
                DocumentSmithBOD(html, system.GetAllRewards(system.ComputePoints(sbod)), "10, 15, 20: Exceptional", sbod.Material);
                ...
                for (BulkMaterialType mat = BulkMaterialType.None; mat <= BulkMaterialType.Valorite; ++mat)
                {
                    sbod.Material = mat;
                    sbod.AmountMax = 10;
                    DocumentSmithBOD(html, system.GetAllRewards(system.ComputePoints(sbod)), "10, 15, 20", sbod.Material);
                }
                ...
                for (BulkMaterialType mat = BulkMaterialType.None; mat <= BulkMaterialType.Valorite; ++mat)
                {
                    ...
                    {
                        sbod.AmountMax = amt;
                        DocumentSmithBOD(html, system.GetAllRewards(system.ComputePoints(sbod)), amt == 20 ? "20" : "10, 15", sbod.Material);
                    }
                }
            ...
            {
                BulkOrderSystem system = DefTailor.System;
                ...
                sbod.Material = BulkMaterialType.None;
                DocumentTailorBOD(html, system.GetAllRewards(system.ComputePoints(sbod)), "10, 15", sbod.Material, sbod.Type);
                ...
                sbod.Type = typeof(SkullCap);
                sbod.Material = BulkMaterialType.None;
                DocumentTailorBOD(html, system.GetAllRewards(system.ComputePoints(sbod)), "20", sbod.Material, sbod.Type);
        ...
        private static void WriteTailorLBOD(StreamWriter html, string name, SmallBulkEntry[] entries, bool expandCloth, bool expandPlain)
        {
            ...
            if (showCloth)
            {
                ...
                {
                    lbod.AmountMax = 10;
                    DocumentTailorBOD(html, system.GetAllRewards(system.ComputePoints(lbod)), "10, 15", lbod.Material, type);
                    lbod.AmountMax = 20;
                    DocumentTailorBOD(html, system.GetAllRewards(system.ComputePoints(lbod)), "20", lbod.Material, type);
                }
                else
                {
                    lbod.AmountMax = 10;
                    DocumentTailorBOD(html, system.GetAllRewards(system.ComputePoints(lbod)), "10, 15, 20", lbod.Material, type);
                }
            }
            ...
            {
                lbod.AmountMax = 10;
                DocumentTailorBOD(html, system.GetAllRewards(system.ComputePoints(lbod)), "10, 15, 20", lbod.Material, typeof(LeatherCap));
                lbod.AmountMax = 20;
                DocumentTailorBOD(html, system.GetAllRewards(system.ComputePoints(lbod)), "20", lbod.Material, typeof(LeatherCap));
            }
            else
            {
                lbod.AmountMax = 10;
        ...
        private static void WriteSmithLBOD(StreamWriter html, string name, SmallBulkEntry[] entries)
        {
            ...
            for (BulkMaterialType mat = BulkMaterialType.None; mat <= BulkMaterialType.Valorite; ++mat)
            {
                lbod.Material = mat;
                lbod.AmountMax = 10;
                DocumentSmithBOD(html, system.GetAllRewards(system.ComputePoints(lbod)), "10, 15, 20", lbod.Material);
            }
            ...
            for (BulkMaterialType mat = BulkMaterialType.None; mat <= BulkMaterialType.Valorite; ++mat)
            {
                ...
                {
                    lbod.AmountMax = amt;
                    DocumentSmithBOD(html, system.GetAllRewards(system.ComputePoints(lbod)), amt == 20 ? "20" : "10, 15", lbod.Material);
                }
            }
        ...
        private static void WriteEnum(TypeInfo info, StreamWriter typeHtml)
        {
            ...
            for (int i = 0; i < values.Length; ++i)
            {
                ...
                typeHtml.WriteLine(format, value, value, i < (values.Length - 1) ? "," : "");
            }
        }
