 ...
 *      Call OnAfterSpawn() after spawning a creature. This provides the opportunity to establish dynamic alignment (based on region.)
 *  10/14/22, Adam (AccessLevelRemap)
...
namespace Server.Commands
{
    public class Add
    {
        ...
        {
            // access level < Owner get the event equivalent of these items: spawners, teleporters, and moongates
            Utility.AccessLevelRemap(from, args);
            ...
            if (props != null)
            {
                ...
                for (int i = 0; i < realProps.Length; ++i)
                {
                    ...
                    else
                    {
                        ...
                        else if ((from != null) && from.AccessLevel < attr.WriteLevel)
                        {
                            if (from != null)
        ...
        {
            List<object> BuiltList = new List<object>();
            try
            {
                Map map = from.Map;
                    ...
                    for (int x = start.X; x <= end.X; ++x)
                    {
                        for (int y = start.Y; y <= end.Y; ++y)
                        {
                            ...
                            if (built is Item item)
                            {
                                ...
                                // only log the creation of interesting things
                                if (item is Spawner || item is ChestItemSpawner || item is ITrigger || item is ITriggerable || item is Teleporter)
                                    item.LogChange(from, message: string.Format($"{item.GetType().Name} added"));
                            }
                            else if (built is Mobile)
                            {
                                Mobile m = (Mobile)built;
            ...
            {
                // command logging
                if (from != null && BuiltList.Count > 0)
                {
                    foreach (object o in BuiltList)
                    {
                        string where = null;
                        if (o is Mobile mobile)
                            where = mobile.Location.ToString();
                        else if (o is Item item)
                            where = item.GetWorldLocation().ToString();
                            
                        string output = string.Format($"{from.AccessLevel.ToString()} {CommandLogging.Format(from)} built {CommandLogging.Format(o)} at {where}");

                        CommandLogging.WriteLine(from, output);
                    }
                }
            }
        }
